...
private fun startApp(packageName: String?, result: Result) {
    try {
        ...
        if (intent != null) {
            ...
            // Không tìm thấy ứng dụng hoặc không thể tạo Intent
            result.error("APP_NOT_FOUND", "Application not found or invalid package name", null)
        }
    } catch (e: Exception) {
        e.printStackTrace()
        result.error("START_APP_ERROR", "Error starting application: ${e.message}", null)
    }
}
...
private fun lockApp(home: Boolean, result: Result?) {
    ...
    var isReplySent = false // Cờ kiểm tra
